...
このドキュメントでは、`work-report-system` のSpring MVC構成を説明します。
...
| `src/main/java/com/example/workreport/controller/LoginController.java` | ログイン、ログアウトのController |
| `src/main/java/com/example/workreport/security/WorkReportUserDetailsService.java` | Spring Securityのユーザー取得処理 |
| `src/main/java/com/example/workreport/controller/DashboardController.java` | ダッシュボード表示のController |
| `src/main/java/com/example/workreport/controller/WorkReportController.java` | 作業日報登録、作業実績検索のController |
| `src/main/java/com/example/workreport/controller/MonthlyReportController.java` | 月次報告書Excel出力のController |
| `src/main/java/com/example/workreport/controller/ReportHistoryController.java` | 帳票作成履歴検索、詳細、再ダウンロードのController |
| `src/main/webapp/WEB-INF/views/home.jsp` | トップ画面JSP |
| `src/main/webapp/WEB-INF/views/login.jsp` | ログイン画面JSP |
| `src/main/webapp/WEB-INF/views/dashboard.jsp` | ダッシュボード画面JSP |
| `src/main/webapp/resources/css/common.css` | 共通CSS |
| `src/main/webapp/WEB-INF/web.xml` | DispatcherServlet、ContextLoaderListener、Spring Security Filterの定義 |
...
`web.xml` には `springSecurityFilterChain` も定義しています。ログイン画面と静的リソース以外のURLは、Spring Securityによって認証済みユーザーだけがアクセスできるようにしています。

## Controller

`HomeController` は `com.example.workreport.controller` パッケージに配置しています。

`GET /home` を受け取り、画面に表示する機能一覧を `Model` に設定して、ビュー名 `home` を返します。
...
`/home` は公開トップ画面のため、DB接続や業務処理は行っていません。業務機能はログイン後の各画面で利用します。
...
たとえば `HomeController` では、主な機能一覧を `features` という名前でModelへ設定しています。
...
| `ReportHistorySearchForm` | 帳票作成履歴検索条件 |
...
- `/home` によるSpring MVC最小疎通
- `/login`、`/dashboard`、`/logout` によるセッション管理
- Formを使った作業日報登録、作業実績検索、月次報告書出力
- ControllerからService、DAOへ処理を委譲するレイヤ構成
- JSP / JSTLによるサーバーサイド画面表示
- `/resources/**` による静的CSS配信
