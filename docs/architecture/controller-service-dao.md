...
認証では `WorkReportUserDetailsService` がログインIDをもとに `UserDao` を呼び出し、`users.password` に保存されたBCryptハッシュをSpring Securityが照合します。認証成功後は `LoginSuccessHandler` が画面表示補助用のユーザー情報とログイン時刻をHTTPセッションに保存します。認証状態の正はSpring SecurityのSecurityContextであり、Controllerの `SessionUtils` もSecurityContextを優先してログインユーザーを取得します。
...
DAOが担当するのはDBに近い処理です。SQL文、バインド変数、`RowMapper` によるDTO/Entity変換、検索結果0件時の扱いなどをDAO内に閉じ込めます。複雑なSELECTは `src/main/resources/sql/dao/` 配下のSQLファイルに置き、DAOは `SqlFileLoader` で読み込んだうえで動的WHERE句を追加します。
...
ステータス表示は、DAOのSELECT内で `SUCCESS` を「完了」、`ERROR` を「エラー」、`PROCESSING` を「処理中」に変換しています。Excel出力は同期処理ですが、出力開始時に `PROCESSING`、成功時に `SUCCESS`、失敗時に `ERROR` へ更新するため、処理状態を履歴として追えるようにしています。

帳票作成履歴は、Service層でログインユーザーの権限を確認します。`ADMIN` は全ユーザー分を扱えますが、`USER` は自分が帳票対象者になっている履歴だけを検索・詳細表示・再ダウンロードできます。ControllerのURLを直接指定されても、Service側で対象外の履歴を `null` として扱うため、他ユーザーのファイルを取得できないようにしています。

作業実績検索も同じ考え方です。`WorkReportController` は検索条件とログインユーザーを `WorkReportService` に渡します。`USER` の場合はServiceがログインユーザーIDをDAOへ渡し、DAOが `wr.user_id = :userId` をWHERE句に追加します。画面の社員名・部署名を改変されても、一般ユーザーが他ユーザーの作業実績を取得できないようにしています。

ログインユーザーを前提にするServiceメソッドは、`loginUser` が渡されない場合に例外を投げます。未ログインや呼び出しミスを「全件参照」などの広い権限として扱わず、安全側に倒す方針です。
...
履歴登録は `ReportHistoryService` でトランザクション管理します。出力開始時に `PROCESSING` で履歴を作成し、成功時は同じ履歴を `SUCCESS` へ、失敗時は `ERROR` へ更新します。Excelファイル保存後に失敗した場合は、生成済みファイルの削除を試み、削除失敗時はログに警告を残します。
