...
`src/main/resources/application.properties` は、Docker Composeで起動したOracle Database Freeに接続する開発用デフォルトを持ちます。Docker Compose側のDBユーザーや管理者パスワードを変更する場合は `.env.example` をコピーして `.env` を作成します。
...
STS上のTomcat 8.5でアプリケーションを起動すると、この接続先を使用してSpring JDBCからOracleへアクセスします。アプリケーション側の接続先を変える場合は、STS/Tomcatの起動構成に `JDBC_URL`、`JDBC_USERNAME`、`JDBC_PASSWORD` などの環境変数を設定します。

`.env` はDocker Compose用のローカル専用ファイルとして `.gitignore` 対象です。リポジトリにはサンプル値を記載した `.env.example` のみを置きます。
