...
履歴登録は `ReportHistoryService` でトランザクション管理します。ファイル保存後にDB登録で失敗した場合は、生成済みファイルを削除します。DBとファイルシステムは完全な単一トランザクションにはできませんが、不要なファイルだけが残る状態を減らす方針です。

現在のExcel出力は同期処理のため、実際に登録するステータスは `SUCCESS` または `ERROR` です。`PROCESSING` は画面表示とDB定義を先に用意している状態で、非同期出力へ拡張する場合は以下の流れにします。

1. 履歴を `PROCESSING` で作成する
2. Excel生成とファイル保存に成功したら `SUCCESS` へ更新する
3. 失敗したら `ERROR` と `error_message` へ更新する
4. ファイル削除失敗もログに残す
5. 同一条件の二重実行や同名ファイルの扱いを運用ルールとして決める
