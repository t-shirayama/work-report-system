...
対象期間に作業実績が0件の場合も、帳票ファイル自体は出力します。この場合、総作業時間、稼働日数、平均作業時間は0として扱い、分類別集計と日別明細はテンプレート上の空行を残します。0件をエラーにはせず、利用者が「該当データなしの月次報告書」として確認できる方針です。
...
履歴を保存する理由は、利用者が後から同じ帳票を再取得できるようにするためです。また、いつ、誰が、誰の、どの対象年月の帳票を作成したかを残すことで、業務上の証跡にもなります。`created_by` は作成操作を行ったユーザー、`target_user_id` は帳票対象ユーザーを表します。
...
詳細画面では、エラーメッセージやファイル情報を確認できます。サーバー内部のファイルパスは管理者だけに表示します。`SUCCESS` は「完了」、`ERROR` は「エラー」、`PROCESSING` は「処理中」として表示します。

帳票作成履歴はログインユーザーの権限に応じて参照範囲を制限します。`ADMIN` は全件を検索できますが、`USER` は `target_user_id` が自分の履歴だけを検索・詳細表示・再ダウンロードできます。これにより、管理者が代理出力した帳票も、対象ユーザー本人が後から参照できます。
